...
**Answer:** $SE[S] = \sqrt{25p(1-p)} (= \sqrt{Np(1-p)})$ 
...
**Answer:** $E[S/N]=E[\bar{X}] =p (= (Np)/N)$

4\. What is the standard error of $\bar{X}$? Hint: it's a function of $p$.

**Answer:** $SE[S/N]=SE[\bar{X}] = \sqrt{p(1-p)/N} (= \sqrt{(Np(1-p))/N^2})$ 
...
```{r}
#SE = sqrt(4*p*(1-p)/N) where p=.45 and N =25
se <- sqrt(4*0.45*(1-0.45)/25)
se
```
...
b.  Our standard error is larger than the difference, so the chances of $2\bar{X}-1$ being positive and throwing us off were not that small. We should pick a larger sample size. **[ANSWER]**
