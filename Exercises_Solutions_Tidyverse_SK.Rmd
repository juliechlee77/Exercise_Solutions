...
c. `co2` is not tidy:  it is a matrix instead of a data frame.XXX
...
b.  `ChickWeight` is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came is one of the variables. XXX
...
c. `BOD` is tidy: each row is an observation with two values (time and demand) XXX
...
```{r}
new <- filter(murders, rate < 1 & region %in% c("West", "Northeast"))
select(new, state, rate, rank)
```
...
NHANES |>
  ...
  group_by(AgeDecade) |>
  summarize(Average = mean(BPSysAve, na.rm = TRUE), Standard_dev = sd(BPSysAve, na.rm = TRUE))
```
...
NHANES |> 
  ...
  group_by(Race1) |>
  summarize(SBP_avg = mean(BPSysAve, na.rm=TRUE)) |>
  arrange(SBP_avg)
```
...
b. `murders` is in tidy format and is stored in a data frame. XXX
...
```
#### Summarize function needs to be told which column name we're looking at
