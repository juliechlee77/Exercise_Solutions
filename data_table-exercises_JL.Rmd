...
```{r}
murders[, rate := total / population_in_millions]
```
...
```{r}
NHANES <- setDT(NHANES)
ref <- NHANES[Gender == "female" & AgeDecade == " 20-29" & !is.na(BPSysAve), .(average = mean(BPSysAve), standard_deviation = sd(BPSysAve))]
```


2\. Report the min and max values for the same group.
```{r}
NHANES[Gender == "female" & AgeDecade == " 20-29", .(min = min(BPSysAve, na.rm = TRUE), max = max(BPSysAve, na.rm= TRUE))]
```
...
```{r}
NHANES[Gender == "female", .(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE)), by = AgeDecade]
```

4\. Repeat exercise 3 for males. 
```{r}
NHANES[Gender == "male", .(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE)), by = AgeDecade]
```
...
```{r}
maleage <- NHANES[Gender == "male" & AgeDecade == " 40-49" & !is.na(BPSysAve), .(average = mean(BPSysAve)), by = Race1]
maleage[order(average)]
```
