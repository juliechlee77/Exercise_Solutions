...
```{r}
NHANES <- setDT(NHANES)
ref <- NHANES[Gender == "female" & AgeDecade == " 20-29", .(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))]
```


2\. Report the min and max values for the same group.
```{r}
NHANES[Gender == "female" & AgeDecade == " 20-29", .(min(BPSysAve, na.rm = TRUE), max(BPSysAve, na.rm= TRUE))]
```
...
```{r}
maleage <- NHANES[Gender == "male" & AgeDecade == " 40-49"]
maleage[order(BPSysAve)]
```
