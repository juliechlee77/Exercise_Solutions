...
d. `co2` is not tidy: to be tidy we would have to wrangle it to have three columns (year, month and value), then each co2 observation would have a row.[**ANSWER**]
...
b.  `ChickWeight` is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came is one of the variables. [**ANSWER**]
...
c. `BOD` is tidy: each row is an observation with two values (time and demand)[**ANSWER**]
...
d. `Formaldehyde`YES
e. `Orange` YES?
f. `UCBAdmissions`NO # has 3 dimensions...it's a 3D array
...
#my_states <- arrange(my_states, rank)
my_states
...
my_states <- filter(murders, rate <=1 & region %in% c("West", "Northeast")) |>
  ...
  arrange(rank)
 # arrange(desc(rank)) # increasing: arrange(rank)
my_states
```
...
NHANES |>
  ...
  group_by(AgeDecade) |>
  summarize(average = mean(BPSysAve, na.rm=TRUE), SD = sd(BPSysAve, na.rm=TRUE))
```
...
```{r}
NHANES |> group_by(AgeDecade, Gender) |> summarize(average = mean(BPSysAve, na.rm = TRUE), SD = sd(BPSysAve, na.rm = TRUE))
```
...
b. `murders` is in tidy format and is stored in a data frame. [**ANSWER?!**, check by class(murders)]
...
murders |> 
  select(population) |> 
  log() |>
  summarize(mean(population)) |> exp()


murders %>%
  summarise(average = exp(mean(log(population)))) %>%
  pull(average)
```
