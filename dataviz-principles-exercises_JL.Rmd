...
```{r}
d <- dat |> mutate(rate = count/population*10000*52/weeks_reporting, state = reorder(state, rate)) 
d |> ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
  coord_flip() 
...
```{r}
m <- murders |> mutate(rate = total/population*100000, region = reorder(region, rate, FUN=median, na.rm=TRUE)) 
