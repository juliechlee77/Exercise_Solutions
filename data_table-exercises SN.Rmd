...
#correct answer
murders[rank<=5, .(state, rate, rank)]
```
...
ref1 <- NHANES %>% 
  filter(AgeDecade %in% " 20-29" & Gender %in% "female") %>%
  select(BPSysAve)
...
# correct answer
nhanes <- setDT(NHANES)
ref2 <- nhanes[AgeDecade == " 20-29" & Gender == "female", .(average = mean(BPSysAve, na.rm=TRUE), standard_deviation = sd(BPSysAve, na.rm=TRUE))]
ref2
```
...
```{r}
min_max <- nhanes[AgeDecade == " 20-29" & Gender == "female", .(minimum = min(BPSysAve, na.rm=TRUE), maximum = max(BPSysAve, na.rm=TRUE))]
min_max
```
...
```{r}
nhanes[Gender == "female", .(average = mean(BPSysAve, na.rm=TRUE), standard_deviation = sd(BPSysAve, na.rm=TRUE)), by = AgeDecade][order(AgeDecade)]
```

4\. Repeat exercise 3 for males. 

```{r}
nhanes[Gender == "male", .(average = mean(BPSysAve, na.rm=TRUE), standard_deviation = sd(BPSysAve, na.rm=TRUE)), by = AgeDecade][order(AgeDecade)]
```
...
```{r}
nhanes[Gender == "male" & AgeDecade == " 40-49", .(BPSys = mean(BPSysAve, na.rm=TRUE)), by = Race1][order(BPSys, decreasing = FALSE)]
```