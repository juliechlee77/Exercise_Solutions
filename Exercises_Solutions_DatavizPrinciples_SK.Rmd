---
title: "Exercises_Solutions_DatavizPrinciples_SK"
author: "Soomin Kim"
date: "2022-10-14"
...
*d. Never. Barplots and tables are always better. XXX*
...
*b. The axis does not start at 0. Judging by the length, it appears Trump received 3 times as many votes when, in fact, it was about 30% more. XXX*
...
*c. The plot on the right is better because alphabetical order has nothing to do with the disease and by ordering according to actual rate, we quickly see the states with most and least rates. XXX*
...
dat |> ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
  coord_flip() 
# geom_bar by default counts the rows of x; if you want to specify y you must write stat="identity"; it will give you an error if you try to specify y without writing stat="identity" 
# vs. geom_col by default you must specify y
```
...
5\. Now with one line of code, define the `dat` table as done above, but use mutate to create a rate variable and re-order the state variable so that the levels are re-ordered by this variable. Then make a barplot using the code above, but for this new `dat`.

```{r}
dat2 <- dat |> mutate(rate = count/population*10000*52/weeks_reporting) |> mutate(state = reorder(state,rate))
dat2 |> ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
  coord_flip()
```
...
*c. It does not show all the data. We do not see the variability within a region and it's possible that the safest states are not in the West.XXX*
...
```{r, eval = FALSE}
data("murders")
d <- murders |> mutate(rate = total/population*100000) |>
mutate(region = reorder(region,rate,FUN=median, na.rm=TRUE))


d |> ggplot(aes(region, rate)) +
...
*a. Humans are not good at reading pseudo-3D plots. XXX*
