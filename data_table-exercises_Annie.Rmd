...
my_states = murders[region %in% c("Northeast", "West") & rate <1,state]
my_states
```
...
library(data.table)
NHANES = setDT(NHANES)
NHANES[AgeDecade ==" 20-29" & Gender =="female",.(mean=mean(BPSysAve),sd=sd(BPSysAve))]
ref = NHANES[AgeDecade ==" 20-29" & Gender=="female", .(mean=mean(BPSysAve,na.rm=T),sd=sd(BPSysAve,na.rm=T))]
```
...
```{r}
NHANES[AgeDecade ==" 20-29" & Gender=="female", .(min=min(BPSysAve,na.rm=T),max=max(BPSysAve,na.rm=T))]
```
...
```{r}
NHANES[Gender == "female", .(mean=mean(BPSysAve,na.rm=T),sd=sd(BPSysAve,na.rm=T)), by= AgeDecade]
```
...
```{r}
NHANES[Gender == "male", .(mean=mean(BPSysAve,na.rm=T),sd=sd(BPSysAve,na.rm=T)), by= AgeDecade]
```
...
```{r}
male= NHANES[Gender == "male" & AgeDecade == " 40-49", .(mSBP=mean(BPSysAve,na.rm=T)), by= Race1]
male[order(mSBP,decreasing = F)]
```
