...
d. Never. Barplots and tables are always better. **[ANSWER]**
...
b. The axis does not start at 0. Judging by the length, it appears Trump received 3 times as many votes when, in fact, it was about 30% more. **[ANSWER]**
...
c. The plot on the right is better because alphabetical order has nothing to do with the disease and by ordering according to actual rate, we quickly see the states with most and least rates. **[ANSWER]**
...
c. It does not show all the data. We do not see the variability within a region and it's possible that the safest states are not in the West. **[ANSWER]**
...
murders |>
  mutate(rate = total/population*100000) |>
  group_by(region) |>
  ggplot(aes(reorder(region, rate, na.rm = TRUE), rate)) + #CHECK - how to reorder regions?
  geom_boxplot() +
  xlab("Region") +
  geom_jitter()
...
p7_order <- murders %>%
  ...
  geom_boxplot(coef=3) + # what does this coefficient mean?
...
a. Humans are not good at reading pseudo-3D plots.**[ANSWER]**
