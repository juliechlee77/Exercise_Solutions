...
b. The graph above shows only four numbers with a bar plot. **[ANSWER]**
...
d. With the exception of 4 states, the murder rates are below 5 per 100,000. **[ANSWER]**
...
d. They are the same dataset, but the first is not in the log scale, the second undersmooths, and the third oversmooths. **[ANSWER]**
...
average <- mean(x)
SD <- sd(x)
plot(pnorm(seq(min(x), max(x)), average, SD))
...
```{r}
# different than Ryan?
(mean(x <= 81) - mean(x <= 79)) / (pnorm(81, average, SD) - pnorm(79, average, SD))
```
...
c. As seen in question 10, the normal approximation tends to underestimate the extreme values. It's possible that there are more seven footers than we predicted. **[ANSWER]**
