...
```{r}
(0.5)^4+4*0.5*(0.5)^4+(5*4/2)*(0.5)^2*(0.5)^4
```
...
```{r, eval=FALSE}
#the probability of winning the series for the underdog team B (chance: 1-p)

prob_win <- function(p){
  B <- 10000
  result <- replicate(B, {
    b_win <- sample(c(1,0), 7, replace = TRUE, prob = c(1-p, p))  #so here, prob = c(1-p, p)
    sum(b_win)>=4                                                #rather than prob = c(p, 1-p)
  })
  mean(result)
}
```
...
```{r}
(pnorm(67,64,3)-pnorm(64,64,3))+(pnorm(64,64,3)-pnorm(61,64,3))
```
...
highest <- function(N){
  B <- 1000
  result <- replicate(B, {
    simulated_IQ <- rnorm(n, 100, 15)
    max(simulated_IQ)
  })
}

data.frame(highest = highest(10000)) |> ggplot(aes(highest)) + 
...
data.frame(smartest = smartest) |> ggplot(aes(smartest)) + 
  geom_histogram(color="black", binwidth = 1) 
```
