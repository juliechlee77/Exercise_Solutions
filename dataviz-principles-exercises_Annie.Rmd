...
```{r}
dat |> mutate(rate = count / population * 10000 * 52 / weeks_reporting, state = reorder(state,rate)) |> ggplot(aes(state,rate)) + geom_bar(stat="identity") + coord_flip()
```
...
murders |> mutate(rate = total/population*100000) |> 
  mutate(region=reorder(region, rate, FUN = median))|>
    ggplot(aes(region,rate)) + geom_boxplot() + geom_point() + geom_jitter(width = 0.1, alpha = 0.2)
