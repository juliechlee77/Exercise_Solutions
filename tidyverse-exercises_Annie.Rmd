...
```{r}
my_states = murders |> filter(rate <=1 & region %in% c("West", "Northeast"))|>
...
ref = NHANES |> filter(AgeDecade == " 20-29" & Gender == "female") |>
  summarise(mean=mean(BPSysAve,na.rm=T), standard_deviation = sd(BPSysAve,na.rm=T))
ref
```
...
```{r}
ref_minmax = NHANES |> filter(AgeDecade == " 20-29" & Gender == "female") |>
  summarise(min=min(BPSysAve,na.rm=T), max = max(BPSysAve,na.rm=T))
...
```{r}
all_f = NHANES |> filter(Gender =="female") |> group_by(AgeDecade) |>
  summarise(avg = mean(BPSysAve,na.rm=T), sd = sd(BPSysAve,na.rm=T))
all_f
```
...
```{r}
all_m = NHANES |> filter(Gender =="male") |> group_by(AgeDecade) |>
  summarise(avg = mean(BPSysAve,na.rm=T), sd = sd(BPSysAve,na.rm=T))
all_m
```
...
```{r}
all = NHANES |> group_by(AgeDecade,Gender) |>
  summarise(avg = mean(BPSysAve,na.rm=T), sd = sd(BPSysAve,na.rm=T))
all
```
...
NHANES |> filter(Gender == "male" & AgeDecade ==" 40-49") |> group_by(Race1) |> 
  summarise(SBP = mean(BPSysAve, na.rm=T)) |> arrange(SBP)
```
...
```{r}
murders |> group_by(region) |> as_tibble()
```
...
```{r, eval=FALSE}
exp(mean(log(murders$population)))  # I prefer this one
```
...
```{r}
murders |> select(population) |> log() |> summarise(mean(population)) |> exp()
```
...
compute_s_n <- function(n){
  x <- 1:n
  tibble(n=n, s_n= sum(x), s_n_2= paste(sum(x),"row number=", n))
}
